  ...
  try {
    ...
    for (const part of proposal.parts) {
      ...
      const description = buildPRDescription(
        ...
        headBranch,
        part.rationale,
        part.order === 1 && proposal.parts.length > 1
      );
...
function buildPRDescription(
  ...
  rationale: string,
  includesGeneratedChainWorkflowDiff: boolean
): string {
  const generatedWorkflowNote = includesGeneratedChainWorkflowDiff
    ? `
### Workflow cleanup
