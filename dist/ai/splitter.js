...
import { buildSystemPrompt, buildSplitPrompt, buildMergePrompt, parseAIResponse, } from "./prompt.js";
/**
 * AIを使ってPRの分割提案を生成する
 * diffが大きい場合は自動でチャンク分割して複数回AIに投げ、結果を統合する
 */
export async function generateSplitProposal(client, options, onProgress) {
    ...
    if (chunks.length === 1) {
        ...
        const userPrompt = buildSplitPrompt(prTitle, prBody, files, additionalInstruction);
        const response = await client.complete(systemPrompt, userPrompt);
        return parseAIResponse(response);
    }
    // 複数チャンクの場合：分割送信 → 統合
    onProgress?.(`diffが大きいため ${chunks.length} チャンクに分割して処理します...`);
    const chunkResults = [];
    for (let i = 0; i < chunks.length; i++) {
        onProgress?.(`チャンク ${i + 1}/${chunks.length} を処理中...`);
        const userPrompt = buildSplitPrompt(prTitle, prBody, chunks[i], additionalInstruction);
        const response = await client.complete(systemPrompt, userPrompt);
 ...
 */
export function validateProposal(proposal, originalFiles) {
    const errors = [];
    const originalFilenames = new Set(originalFiles.map((f) => f.filename));
    const proposedFilenames = new Set(proposal.parts.flatMap((p) => p.files));
    // 元のファイルがすべてカバーされているか
    ...
    for (const part of proposal.parts) {
        for (const filename of part.files) {
            if (seen.has(filename)) {
                errors.push(`ファイル "${filename}" が重複して複数の分割PRに含まれています。`);
            }
            seen.add(filename);
        }
    }
    ...
    for (const filename of proposedFilenames) {
        if (!originalFilenames.has(filename)) {
            errors.push(`ファイル "${filename}" は元PRに含まれていませんが、分割案に存在します。`);
        }
    }
    // order の連番チェック
