...
program
    ...
    .option("--model <model>", "使用するAIモデル (claude|codex)", "claude")
    .option("--dry-run", "分割案のみ表示、PR作成はしない", false)
    .action(async (prIdentifier, opts) => {
    const model = opts.model;
    const dryRun = opts.dryRun;
    let additionalPrompt = opts.prompt;
    // モデルのバリデーション
    if (!["claude", "codex"].includes(model)) {
        console.error(chalk.red(`エラー: 無効なモデル "${model}"。claude または codex を指定してください。`));
        process.exit(1);
    }
...
function displayProposal(proposal) {
    ...
    for (const part of proposal.parts) {
        ...
        if (part.files.length <= 5) {
            ...
            console.log(chalk.dim(`       ... 他 ${part.files.length - 3} ファイル`));
        }
    }
    console.log();
}
 ...
 * 作成されたPR一覧を表示する
 */
function displayCreatedPRs(prs) {
    console.log();
    for (const pr of prs) {
